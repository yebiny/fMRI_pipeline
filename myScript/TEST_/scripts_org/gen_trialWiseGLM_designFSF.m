...
function gen_trialWiseGLM_designFSF(subj, path)
%%
nEV = [110 10];
...
for run = 1:10
    ...
    for trainTest = 1:length(trainTest_list)
        fileName = [path.fsf '/phase1_trialWiseGLM_' trainTest_list{trainTest} '_run' num2str(run) '.fsf'];
        ...
        fprintf(fid, '%s\n', '# Output directory');
        xStr = [path.analysis.firstlevel '/phase1_trialWiseGLM_' trainTest_list{trainTest} '_run' num2str(run)];
        ...
        xStr = [path.nifti '/primeRecon_' subj.SN '_train_' xRun];
        ...
        xStr = [path.nifti '/primeRecon_' subj.SN '_t1_mprage_brain'];
        ...
        for ii = 1:nEV(trainTest)
            ...
            xStr = ['set fmri(custom' num2str(ii) ') "' path.designRoot '/phase1_run' num2str(run) '/onset_' trainTest_list{trainTest} num2str(ii) '.txt"'];
            fprintf(fid, '%s\n', xStr);
            fprintf(fid, '\n');
